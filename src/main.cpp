    ...
    try
    {
        for (int i = 1; i < argc; ++i) 
        {
            ...
            if (arg == "-help" || arg == "--help") 
            {
                ...
                "code_rate_adaptation_parameters - Parameters for adaptation of parity-check\n"
                "   matrices code rate.\n"
                "   enable_untainted_puncturing - Enables (when code rate(R) modulation enabled)\n"
                "       the use of bits obtained using the algorithm for puncturing untainted\n"
                "       nodes as punctured symbols. Otherwise, random positions of punctured nodes\n"
                "       are used. More info: https://ieeexplore.ieee.org/document/6290312.\n"
                "   use_adaptation_parameters_ranges - For all matrices, δ and f_EC values will be\n"
                "       generated based on the ranges specified by 'code_rate_adaptation_param-\n"
                "       eters_ranges'.\n"
                "   code_rate_adaptation_parameters_ranges - Code rate(R) and delta(δ) with\n" 
                "       reconciliation efficiency(f_EC) correspondence ranges. The 'code_rate' value\n"
                "       of parity-check matrix correspond to a range of δ and f_EC values from\n"
                "       'begin' to 'end' in 'step' increments. If 'begin' == 'end', only one value\n"
                "       will be used, and 'step' will not be taken into account.\n"
                "   code_rate_QBER_adaptation_parameters_maps - Code rate(R), QBER, delta(δ) and\n"
                "       reconciliation efficiency(f_EC) mappings. Used when 'use_adaptation_param-\n"
                "       eters_ranges' is false to use specific QBER values and adaptation parameters\n"
                "       for certain code rate. Multiple entries can be provided for the same\n"
                "       code rate. In this case, 'code_rate_QBER_ranges' is not used.\n\n"
