    ...
    try
    {
        for (int i = 1; i < argc; ++i) 
        {
            ...
            if (arg == "-help" || arg == "--help") 
            {
                ...
                "interactive_mode - If true, the user can select a parity check matrix and run\n" 
                "   tests with it. Otherwise, BATCH MODE is used, which reads all parity-check\n"
                "   matrices from the specified directory, runs tests with them, and writes the\n"
                "   results to a file.\n\n"
                ...
                "   'QBER_begin' to 'QBER_end' in 'QBER_step' increments. If\n"
                "   'QBER_begin' == 'QBER_end', only one value will be used, and 'step' will\n"
        ...
        std::vector<fs::path> config_paths = get_file_paths_in_directory(CONFIG_DIR_PATH);
